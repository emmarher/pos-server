<#
  installer/windows/bin/instalar-garage.ps1 — Instala Docker + Garage con un doble clic.

  Fases (ver README-INSTALADOR.md §8):
    0-chequeos → 1-wsl → 2-docker → 3-garage → 4-verificar → listo
  Estado persistente: %ProgramData%\POS Server\garage\install-state.txt
  Log:                %ProgramData%\POS Server\garage\install-garage.log
  ...
  Códigos de salida (para soporte: "¿en qué paso se quedó?"):
    ...
    13 Docker falló | 14 daemon sin arrancar | 15 garage-init falló |
    20 reinicio programado (vuelve solo)

  Uso: instalar-garage.bat  (o)  instalar-garage.ps1 [-Silent]
...
function Register-Resume() {
  ...
  Set-ItemProperty -LiteralPath $key -Name 'POSGarageInstall' -Value $cmd
...
Write-Step '=== Instalador de imágenes (Garage) ==='
...
if ((-not $Silent) -and ($phase -eq 'chequeos')) {
  ...
  Write-Output '  3. Garage (almacén de fotos) + configuración automática'
...
# ── Fase 3: Garage (compose + bootstrap idempotente existente) ─────────────
if ($phase -eq 'garage') {
  Write-Step '[4/4] Garage...'
  if (-not (Test-Path -LiteralPath (Join-Path $GarageDir 'docker-compose.yml'))) {
    Write-Step ("ERROR: no existe {0} (reinstala POS Server)." -f (Join-Path $GarageDir 'docker-compose.yml'))
    exit 15
  }
  & powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $AppDir 'bin\garage-init.ps1')
  if ($LASTEXITCODE -ne 0) { Write-Step 'ERROR: garage-init falló (ver mensajes arriba).'; exit 15 }
  Write-Step 'Garage configurado.'
  Set-Phase 'verificar'
  $phase = 'verificar'
}
...
if ($phase -eq 'verificar') {
  ...
  if (-not $envOk) { Write-Step 'ERROR: el .env del server no tiene S3_* (¿falló garage-init?).'; exit 15 }
  Set-Phase 'listo'
  Write-Step ''
  Write-Step '=== IMÁGENES LISTAS ==='
  Write-Output 'Garage corriendo (auto-arranque por Docker), bucket y key listos,'
